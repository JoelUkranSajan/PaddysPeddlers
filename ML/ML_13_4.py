...
import numpy as np
import pickle
from weather_forecast_api_test import weather_api_request_forecast
...
def data_clean_and_separation():
    df = pd.read_csv('ML_Raw_Input.csv')

    Selected_Columns = df[['number', 'scrape_time', 'temperature', 'humidity', 'visibility', 'windspeed', 'available_bikes']].copy()
...
def modelPickler(models_dict):
    for i in range(1, len(models_dict)+1):
        try:
            ...
            modelFileName = 'rf_model'+ str(i) +'.pkl'
...
# def trip_points(FrontendJSON):

#     Input = FrontendJSON['list']
#     flattened_input = []
...
#     differenceStart = (forecast_df.columns.astype(int) - startUnixTimestamp).abs()
#     closestEpochColumnIndexStart = differences.idxmin()
#     forecastRowStart = df.loc[closestEpochColumnIndexStart]

#     differenceEnd = (forecast_df.columns.astype(int) - endUnixTimestamp).abs()
#     closestEpochColumnIndexEnd = differences.idxmin()
#     forecastRowEnd = df.loc[closestEpochColumnIndexEnd]
    

#     toPredict1 = pd.DataFrame()
#     toPredict2 = pd.DataFrame()

    

#     toPredict1['month', 'hour', 'day_of_week', 'windspeed', 'temperature', 'humidity', 'visibility'] = [finput['month1'], finput['hour1'], finput['day_of_week'], finput['month1'], finput['month1'], finput['month1'], finput['month1']]
#     toPredict2['month', 'hour', 'day_of_week', 'windspeed', 'temperature', 'humidity', 'visibility'] = [finput['month1'], finput['hour1'], finput['day_of_week'], finput['month1'], finput['month1'], finput['month1'], finput['month1']]
...
#     flattened_input.append(flattened_forecast)
    
#     model1 = model_creation_single(StationNumber1)
#     model2 = model_creation_single(stationNumber2)




# print(generate_model_and_coords(23))
...
'''
coordsTuple = generate_model_and_coords(2)
plt.figure(figsize=(10,5))
#plt.plot(coordsTuple[0], coordsTuple[1])

#plt.show()


print(len(coordsTuple[0]))
# 300 represents number of points to make between T.min and T.max
xnew = np.linspace(coordsTuple[0][0],coordsTuple[0][-1],  300)  

spl = make_interp_spline(coordsTuple[0], coordsTuple[-1], k=3)
