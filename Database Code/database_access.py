...
def extract_stations(jc_data_list):
...
def extract_availability(jc_data_list):
    ...
    for entry in jc_data_list:
        ...
        if existing_entry:
            ...
            try:
                ...
                availability = Availability(number, last_update, available_bikes, available_bike_stands, status)
    ...
    if not existing_entry:
        try:
            ...
            weather = Weather(station_id, weather_main, weather_description, temperature, feels_like, pressure,
                        humidity, visibility, windspeed, clouds, time, sunrise, sunset)
            session.add(weather)
            session.commit()
...
# and the subsequent code exports this as a JSON file for use by the front end.
    ...
    with open("query_result.json", 'w') as file:
        result.to_json("query_result.json", orient="records")

# END OF FUNCTION

################################################################################################################

# Note this function is used to clear the database while troubleshooting during initial setup.
